...
class BannerController extends Controller
{
    ...
    public function banner_add_post(Request $request)
    {
        ...
        if($request->hasfile('upload')){
            ...
            foreach($banner_size as $index => $size){
                ...
                $upload[$index]->move(public_path('banner_collection'), $file_name);
    ...
    public function project_addon_post(Request $request, $id)
    {
        ...
        if($version_request!= 0){
            ...
            if($version_request == 1){
                //check version exists or not
                //if null then create one
                $version_info = Version::where('project_id', $project_info['id'])->first();
    ...
    public function editBannerVersion($project_id, $id)
    {
        ...
        $version_name = $version_info['title'];
        $size_list = BannerSizes::orderBy('width', 'ASC')->get();
        return view('view_banner.version_banner.banner-edit', compact('main_project_id', 'version_id', 'version_name', 'project_name', 'size_list'));
    }
