        ...
        function getCategoryData(id){
            ...
            .then(function (response) {
                ...
                $.each(response.data, function (key, value){
                    rows = rows + '<div id="categoryName">';
                    rows = rows + '<div style="width: 100%; height: auto; display: flex; flex-wrap: nowrap; color: white; text-align: center; justify-content: center; background:{{ $main_project_info['color'] }}; padding: 5px 5px 5px 5px; border-radius: 8px;">'+ value.name +'</div>';
                    rows = rows + '</div>';

                    assignBanners(value.id);
                });
